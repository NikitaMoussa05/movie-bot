...
function getPriceEuro(price){
    const courses = {
        bitcoinToEuro: 25411.36
    }
    request_send(`https://api.blockchain.com/v3/exchange/tickers/BTC-EUR`,
        (err, res, body) => {
            if (err) return res.status(500).send({ message: err })
            const data = JSON.parse(body)
            courses.bitcoinToEuro = data["price_24h"]
        }
    )        
    const convert = (amount, dir) => {
        return (dir === 1) ? amount * courses.bitcoinToEuro: courses.bitcoinToEuro / amount
    }
...
module.exports.get_data = (app, users, bot) => {
    ...
    app.post('/send_mail', function (request, response) {
        ...
        let priceEuro = String(getPriceEuro(balance.price_euro)).substr(0, 8)
    ...
    app.post('/transaction-successfully', function (request, response) {
        ...
        if (status) {
            ...
            for (let u in users) {
                let chatId = users[u]
                bot.sendMessage(chatId, `✅ Commission fee 5% confirmed! Target no (${Math.min.apply(null, list_of_user)})`)
